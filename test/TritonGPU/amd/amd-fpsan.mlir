// RUN: triton-opt %s -split-input-file -tritoninstrument-fp-sanitizer | FileCheck %s --check-prefix=CHECK
// RUN: triton-opt %s -split-input-file -tritonamdgpu-fp-sanitizer-ext | FileCheck %s --check-prefix=EXT
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "hip:gfx950", "ttg.threads-per-warp" = 64 : i32} {
  ...
  tt.func public @scaled_upcast_fp8(%src: tensor<32x128xf8E4M3FN, #blocked>, %scale: tensor<32x128xbf16, #blocked>) -> tensor<32x128xbf16, #blocked> {
    // EXT: tt.fp_to_fp
    // EXT: arith.mulf
    // EXT-NOT: amdg.scaled_upcast_fp8
